...
public class SalesScrap extends AbstractFunction implements ITablePage {
	...
	protected String onDelete(HttpServletRequest req, HttpServletResponse resp, FunctionItem fi,
		...
		try {
			DeleteInfo info = delete(fi, sales_id, lm_time, comment);
			conn.commit();
			return Stringify.getTimestamp(info.lm_time) + C.char_31 + info.lm_user + C.char_31 + SalesStatus.scrapped.getName(l) + C.char_31 + SalesStatus.scrapped.getId();
		} catch (Exception e) {
			conn.rollback();
			throw e;
		}
	...
	public DeleteInfo delete(FunctionItem fi, long sales_id, Timestamp lm_time, String comment) throws Exception {
		L l = fi.getLanguage();
		Connection conn = fi.getConnection().getConnection();
		Timestamp old_lm_time;
		int shop_id, member_id, status_id;
		Integer point_used, wallet_used;
		Freight freight;
		try (PreparedStatementWrapper pstmt = new PreparedStatementWrapper(conn.prepareStatement("select lm_time, shop_id, point_used, member_id, status_id, freight_id, wallet_used from sales where sales_id = ? for update"))) {
			pstmt.setLong(1, sales_id);
			try (ResultSetWrapper r = pstmt.executeQueryWrapper()) {
				if (r.next()) {
					old_lm_time = r.getTimestamp(1);
					shop_id = r.getInt(2);
					point_used = r.getInt(3);
					member_id = r.getInt(4);
					status_id = r.getInt(5);
					freight = Freight.get(r.getInt(6));
					wallet_used = r.getInt(7);
				} else {
		...
		if (status != SalesStatus.created && status != SalesStatus.checkout) {
			if (status == SalesStatus.paid && freight == Freight.cash) {
				// allow paid status for cash to be scrapped
			} else {
		...
		try (PreparedStatementWrapper pstmt = new PreparedStatementWrapper(conn.prepareStatement("update sales set status_id = ?, lm_time = ?, lm_user = ?, lm_time_scrapped = ?, lm_user_scrapped = ?, comment = ? where sales_id = ?"))) {
			pstmt.setInt(1, SalesStatus.scrapped.getId());
		...
		if (point_used != null) {
			int remit_point;
			try (PreparedStatementWrapper pstmt = new PreparedStatementWrapper(conn.prepareStatement("select remit_point from member where member_id = ? for update"))) {
			...
			remit_point = remit_point + point_used;
			...
			try (PreparedStatementWrapper pstmt = new PreparedStatementWrapper(conn.prepareStatement("update member set remit_point = ? where member_id = ?"))) {
				pstmt.setInt(1, remit_point);
				pstmt.setLong(2, member_id);
				pstmt.executeUpdate();
			}
			
			try (PreparedStatementWrapper pstmt = new PreparedStatementWrapper(conn.prepareStatement("insert into member_point (member_id, lm_time, remit_point, sales_id, reason_id) values (?,?,?,?,11)"))) {
				pstmt.setLong(1, member_id);
				pstmt.setTimestamp(2, new_lm_time);
				pstmt.setInt(3, point_used);
